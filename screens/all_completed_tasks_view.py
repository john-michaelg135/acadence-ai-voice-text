...
class AllCompletedTasksView(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        back_btn = ctk.CTkButton(header_frame, text="← Back", width=60, fg_color="transparent", text_color=self.tm.text_main(), 
                                 hover_color=self.tm.bg_card(), font=(self.tm.main_font(), 14), command=lambda: self.show_view_callback("History"))
        back_btn.pack(side="left")
        
        ctk.CTkLabel(header_frame, text="All Completed Tasks", font=(self.tm.main_font(), 22, "bold"), text_color=self.tm.text_main()).pack(side="left", padx=20)
    ...
    def _render_completed_chunk(self, tasks, index, render_id, chunk_size=15):
        ...
        for i in range(index, end):
            ...
            pill = ctk.CTkLabel(card, text="Completed", font=(self.tm.main_font(), 11, "bold"), 
            ...
            ctk.CTkLabel(sub_frame, text=task['priority'], font=(self.tm.main_font(), 11, "bold"), text_color=p_color).pack(side="left", padx=(0, 10))
            ctk.CTkLabel(sub_frame, text=task['subject_name'], font=(self.tm.main_font(), 11), text_color=self.tm.text_sub()).pack(side="left", padx=(0, 10))
