...
class HomeView(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        priority_card = ctk.CTkFrame(main_scroll, fg_color=self.purple_main, corner_radius=15)
        priority_card.pack(fill="x", padx=10, pady=(10, 10))
        
        ctk.CTkLabel(priority_card, text="Priority Card", font=("Arial", 16), text_color=self.text_dark).pack(pady=(15, 5))
        
        p_content = ctk.CTkFrame(priority_card, fg_color="transparent")
        p_content.pack()
        ctk.CTkLabel(p_content, text=f"⚠️ {metrics.get('high_priority_count', 0)}", font=("Arial", 40, "bold"), text_color=self.text_dark).pack()

        ctk.CTkLabel(
            priority_card, 
            text="These are the number of pending\ntask with high priority.", 
            font=("Arial", 12), text_color=self.text_dark
        ).pack(pady=(5, 15))

        # 2. Subjects Card (Restored)
        subjects_card = ctk.CTkFrame(main_scroll, fg_color="#FFFFFF", border_color="#E0E0E0", border_width=1, corner_radius=15)
        subjects_card.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(subjects_card, text="Subjects", font=("Arial", 16), text_color=self.text_dark).pack(pady=(15, 5))
        
        s_content = ctk.CTkFrame(subjects_card, fg_color="transparent")
        s_content.pack()
        ctk.CTkLabel(s_content, text=f"🔖 {metrics['total_subjects']}", font=("Arial", 40, "bold"), text_color=self.text_dark).pack()

        ctk.CTkLabel(
            subjects_card, 
            text="Added subjects will appear as\ncontainers in the bottom.", 
            font=("Arial", 12), text_color=self.text_gray
        ).pack(pady=(5, 15))


        # 3. All Tasks Summary Card
        summary_card = ctk.CTkFrame(main_scroll, fg_color="#FFFFFF", border_color="#E0E0E0", border_width=1, corner_radius=15)
        ...
        ctk.CTkLabel(summary_card, text=f"📋 {tasks_count}", font=("Arial", 50, "bold"), text_color="#1A1A1A").pack(pady=5)
        ctk.CTkLabel(summary_card, text=f"Total Pending Tasks", font=("Arial", 13), text_color="#666666").pack(pady=(5, 30))
        
        # Task Distribution Container
        distrib_frame = ctk.CTkFrame(summary_card, fg_color="#FFFFFF", border_color="#E0E0E0", border_width=1, corner_radius=15)
        ...
        ctk.CTkLabel(distrib_frame, text="Your data distribution appears here.", font=("Arial", 11), text_color="#666666").pack(pady=(0, 20))
        
        if not metrics["subjects"]:
            ctk.CTkLabel(distrib_frame, text="No subjects added.", text_color="#AAAAAA").pack(pady=(0, 20))
        ...
        for idx, sub in enumerate(metrics["subjects"]):
            ...
            ctk.CTkLabel(row_frame, text=f"{sub['name']}  :", font=("Arial", 13), text_color="#666666").pack(side="left")
            ctk.CTkLabel(row_frame, text=f" {sub['task_count']}", font=("Arial", 13), text_color="#1A1A1A").pack(side="left")
        ...
        # Row 1: Major
        if majors:
            ctk.CTkLabel(main_scroll, text="Major Subjects", font=("Arial", 12, "bold"), text_color=self.text_dark).pack(anchor="w", padx=15, pady=(10, 0))
            ...
            for sub in majors:
                ...
                ctk.CTkLabel(c, text="📖", font=("Arial", 32), text_color="#FFFFFF").pack(expand=True)
                ctk.CTkLabel(c, text=f"{sub['task_count']} tasks", font=("Arial", 11), text_color="#666666").pack(pady=(5, 10))

        # Row 2: Minor
        if minors:
            ctk.CTkLabel(main_scroll, text="Minor Subjects", font=("Arial", 12, "bold"), text_color=self.text_dark).pack(anchor="w", padx=15, pady=(10, 0))
